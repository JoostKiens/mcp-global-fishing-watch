...
| Structured error handling / error classification                       | **Adopt.** `gfw-client` should produce a small closed set of typed errors (client error / GFW error, with timeout distinguished as `status === 524` within it / rate-limited), not leak raw exceptions or raw GFW JSON to tool callers.                                     |
